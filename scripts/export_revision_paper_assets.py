...
def main():
    ...
    md = f"""# Additional Experimental Results (reviewer supplementary material)

Extra experiments addressing review comments (**SDNET2018-D**, binary image patches). Splits match training scripts: `seed=42`, same logic as `dataset.split_dataset`. Tables report **test** metrics (consistent with `runs_v3/*/results.json`).

Figures are written under `outputs/revision/figures/` (PDF for LaTeX, PNG for Word/Markdown preview).

---

## Table R1 — Loss and augmentation ablation (ResNet18)

{chr(10).join(table_r1_rows)}

**Notes:** *Weighted CE* uses inverse-frequency class weights on the training set with `WeightedRandomSampler` disabled; *Baseline CE* uses standard augmentation + weighted sampling.

---

## Figure R1 — Table R1 (Precision / Recall / F1)

![Figure R1](figures/fig_R1_reviewer_ablation.png)

LaTeX: `\\includegraphics[width=\\linewidth]{{figures/fig_R1_reviewer_ablation.pdf}}`

---

## Table R2 — Focal Loss $\\gamma$ sweep

Fixed $\\alpha{{=}}0.75$, architecture ResNet18 + CBAM + robust augmentation, `no_weighted_sampler`.

{chr(10).join(table_r2_rows)}

---

## Figure R2 — $\\gamma$ vs Acc / Prec / Rec / F1 / FPS (Table R2)

![Figure R2](figures/fig_R2_focal_gamma_sweep.png)

LaTeX: `\\includegraphics[width=0.48\\linewidth]{{figures/fig_R2_focal_gamma_sweep.pdf}}`

---

## Additional supplementary figures (copied from the existing pipeline)

| File | Summary |
| :--- | :--- |
| `fig_sup_training_curves.pdf` | Train/val loss and accuracy |
| `fig_sup_metric_curves.pdf` | Val F1 and recall vs epoch |
| `fig_sup_legacy_ablation_bars.pdf` | Legacy `exp2_*` stepwise ablation bars |
| `fig_sup_cross_backbone.pdf` | Full method across backbones |
| `fig_sup_complexity.pdf` | Params vs FPS scatter |
| `fig_sup_dataset_distribution.pdf` | Crack share by category |

---

## LaTeX row example (decimals, not percentages)

```latex
{latex_example}
```

Combine with booktabs tables as needed; if your manuscript uses percentage columns, multiply values by $100$ and format with `siunitx`.
...
*Values generated by `python scripts/export_revision_paper_assets.py` (reads `runs_v3/*/results.json`).*
"""
